...
# ===================================================================
# TAB 2: ENROLAMIENTO DE PERSONAL
# ===================================================================
            ...
            if not e_name or not e_dni:
                ...
                try:
                    if capture_method == "Cámara Web" and e_camera is not None:
                        ...
                        res = requests.post(f"{FASTAPI_URL}/api/faces/enroll_b64", json=payload, timeout=5)
                    else:
                        files = {"file": (e_file.name, e_file, e_file.type)}
                        data = {"name": e_name, "dni": e_dni, "role": e_role}
                        res = requests.post(f"{FASTAPI_URL}/api/faces/enroll", params=data, files=files, timeout=5)
                ...
                if not persons:
                    ...
                    for p in persons:
                        with st.expander(f"{p['name']} (DNI: {p['dni']}) - {p['role']}"):
                            st.write(f"**Registrado el:** {p['created_at']}")
                            if st.button(f"Eliminar a {p['name']}", key=f"del_{p['id']}"):
                                try:
                                    del_res = requests.delete(f"{FASTAPI_URL}/api/faces/{p['id']}", timeout=2)
...
# ===================================================================
# TAB 3: REGISTRO DE DETECCIONES
# ===================================================================
with tab_registro:
    col_reg1, col_reg2 = st.columns([4, 1])
    with col_reg1:
        st.subheader("Historial de Detecciones e Identificaciones")
        st.markdown("Aquí se guardan las capturas de las personas detectadas por el sistema.")
    with col_reg2:
        ...
        if res.status_code == 200:
            ...
            if not logs:
                ...
                for i, log in enumerate(logs):
                    with cols[i % 3]:
                        st.markdown(f"**Hora:** {log.get('timestamp')}")
